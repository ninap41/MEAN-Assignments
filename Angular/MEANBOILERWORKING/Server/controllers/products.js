...
findproduct: function(req, res){
    console.log("HIT THE FIND PRODUCT CONTROLLER")
    Product.findOne({_id: _id}, function(error, singleproduct){
        console.log(_id)
        console.log(req.params.id)
        // var Product = new Product({postedBy: req.body.poster, text: req.body.text, user_ : req.params.id});
...
update: function(req,res) {
    Product.update({_id:req.body._id}, function(err){ //body not params
        if (err){
            ...
            var product = new Product(req.params.id);
